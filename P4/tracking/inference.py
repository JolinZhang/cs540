    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             captured).
        """
        noisyDistance = observation # by sensor
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob * oldPosDist[p]
        # print "jail positon: ", self.getJailPosition()
        # allPossible is a dict store cords. and prob
        allPossible.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #indexCounter is the index of self.legalPositions
        for x in range(len(self.legalPositions)):
            for y in range(len(self.legalPositions)):
